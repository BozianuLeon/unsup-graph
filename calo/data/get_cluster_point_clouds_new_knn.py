...
for i in range(len(inference)):
    ...
    if h5f.decode('utf-8')=="01":
        ...
        ax.scatter(truth_box_cells_i['cell_xCells'], truth_box_cells_i['cell_zCells'], truth_box_cells_i['cell_yCells'], c='b', marker='o',s=4*cell_significance,alpha=0.6)
        for src, dst in edge_index.t().tolist():
            ...
            ax.plot([x_src, x_dst], [z_src, z_dst], [y_src, y_dst], c='r',lw=0.5,alpha=0.5)

        ax.set(xlabel='X',ylabel='Z',zlabel='Y',title=f'Graph with variable KNN')
        # plt.show()
        fig.savefig(f'../plots/{truth_box_number}/knn-new-cells-graph.{image_format}',dpi=400,format=image_format,bbox_inches="tight")





        feature_matrix = truth_box_cells_2sig_i[['cell_xCells','cell_yCells','cell_zCells','cell_E','cell_eta','cell_phi','cell_Sigma','cell_IdCells','cell_TimeCells']].view(np.float32).reshape(truth_box_cells_2sig_i.shape + (-1,))
        # edge_index = torch_geometric.nn.knn_graph(torch.tensor(feature_matrix[:, :3]),k=3)
        ...
        fig = plt.figure(figsize=(8, 8))
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(truth_box_cells_2sig_i['cell_xCells'], truth_box_cells_2sig_i['cell_zCells'], truth_box_cells_2sig_i['cell_yCells'], c='b', marker='o',s=4*cell_significance,alpha=0.6)
        for src, dst in edge_index.t().tolist():
            x_src, y_src, z_src = truth_box_cells_2sig_i[src][['cell_xCells','cell_yCells','cell_zCells']]
            x_dst, y_dst, z_dst = truth_box_cells_2sig_i[dst][['cell_xCells','cell_yCells','cell_zCells']]
            
            ax.plot([x_src, x_dst], [z_src, z_dst], [y_src, y_dst], c='r',lw=0.5,alpha=0.5)

        ax.set(xlabel='X',ylabel='Z',zlabel='Y',title=f'Graph with variable KNN, {cell_significance_cut}sigma cells')
        # plt.show()
        fig.savefig(f'../plots/{truth_box_number}/knn-new-cells-graph-{cell_significance_cut}sig.{image_format}',dpi=400,format=image_format,bbox_inches="tight")
        ...
        for cl_idx in range(len(cluster_cells_i)):
            ...
            a.scatter(cluster_inside_box['cell_xCells'], cluster_inside_box['cell_zCells'], cluster_inside_box['cell_yCells'], marker='^',s=8)

        a.set(xlabel='X',ylabel='Z',zlabel='Y',title=f'Topocluster cells')
        f.savefig(f'../plots/{truth_box_number}/topocluster-cell-graphs.{image_format}',dpi=400,format=image_format,bbox_inches="tight")
