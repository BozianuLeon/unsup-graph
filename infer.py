...
if __name__=='__main__':
    ...
    dt = np.dtype([('event_no', 'i4'), ('cell_ID', 'f4', (config["max_num"],1)), ('cluster_assign', 'f4', (config["max_num"],))])
    ...
    with torch.inference_mode():
        for step, data in enumerate(test_loader):
            ...
            total_pred_class,total_cell_id,event_nos = [],[],[]
            for batch_idx in range(config["BS"]):
                ...
                total_pred_class.append(predicted_classes)
                total_cell_id.append(y_i)
                event_nos.append(step*config["BS"]+batch_idx)

            # TODO: Need to work out how to save the cluster information here
            # also including the topocluster information - maybe in metrics?
            dataset_idx = step*config["BS"]
            print([len(class_array) for class_array in total_pred_class])
            pred_class_pad = [np.pad(class_array, ((0,config["max_num"]-len(class_array))), 'constant', constant_values=(0)) for class_array in total_pred_class]
            cell_ids_pad = [np.pad(id_array, ((0,config["max_num"]-len(id_array)),(0,0)), 'constant', constant_values=(0)) for id_array in total_cell_id]

    
            inference['event_no'][dataset_idx:dataset_idx+config["BS"]] = event_nos
            inference['cell_ID'][dataset_idx:dataset_idx+config["BS"]]  = cell_ids_pad
            inference['cluster_assign'][dataset_idx:dataset_idx+config["BS"]]  = pred_class_pad
